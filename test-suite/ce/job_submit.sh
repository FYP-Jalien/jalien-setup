...
most_recent_jdl=$(sudo docker exec -t "$CONTAINER_NAME_CE" bash -c "ls -t $directory_path/*.jdl 2>/dev/null | head -n 1")
...
validate_content(){
    content=$(sudo docker exec -t "$CONTAINER_NAME_CE" /bin/bash -c "grep '$1' '$most_recent_jdl'")
    if [ -n "$content" ]; then
...
validate_file(){
    if sudo docker exec -t "$CONTAINER_NAME_CE" /bin/bash -c "test -e '$1'"; then
