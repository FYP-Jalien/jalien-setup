...
replica_host="JCentral-dev.localhost"
...
function write_db_config() {
    ...
    sed -i -e "s:127.0.0.1:${replica_host}:g" $CE_CONFIG/alice_data.properties
    sed -i -e "s:127.0.0.1:${replica_host}:g" $CE_CONFIG/alice_users.properties
...
cat > $CE_CONFIG/custom-classad.jdl << EOF
use_x509userproxy = false
environment = "JALIEN_CM_AS_LDAP_PROXY='localhost.localdomain' JALIEN_HOST=$replica_host JALIEN_WSPORT=8097 ldap_server=$replica_host:8389 JALIEN_PORT=8098 ldap_root=o=localhost,dc=localdomain alien.users.basehomedir=/localhost/localdomain/user/ apiService=$replica_host:8098 trusted.certificates.location=/jalien-dev/certs/globus/CA host.cert.priv.location=/jalien-dev/certs/globus/host/hostkey.pem host.cert.pub.location=/jalien-dev/certs/globus/host/hostcert.pem user.cert.priv.location=/jalien-dev/certs/globus/user/userkey.pem user.cert.pub.location=/jalien-dev/certs/globus/user/usercert.pem alice_close_site=JTestSite jAuthZ.priv.key.location=/jalien-dev/certs/globus/authz/AuthZ_priv.pem jAuthZ.pub.key.location=/jalien-dev/certs/globus/authz/AuthZ_pub.pem SE.priv.key.location=/jalien-dev/certs/globus/SE/SE_priv.pem SE.pub.key.location=/jalien-dev/certs/globus/SE/SE_pub.pem JALIEN_IGNORE_STORAGE=true"
EOF
